"""
Проверка на надёжность, четвёртый эпизод (2026-09-05, Claude/BACKLOG.md #167):
21.04.2025-20.02.2026 -- сильный растущий рынок (VTI +35.87%), недавний
(фундаментал-2026 отстаёт всего на несколько месяцев -- минимальный разрыв
среди всех проверок серии), не пересекается с эпизодом №1 (01.04-05.09.2026)
и заканчивается ровно перед началом медвежьего февраля 2026.
"""
...
CACHE = str(CACHE_DIR / 'signal_frames_2025_2026_bull.pkl')
try:
    with open(CACHE, 'rb') as f:
        frames = pickle.load(f)
    print(f"Кэш найден: {len(frames)} тикеров")
except FileNotFoundError:
    print("Скачиваю историю цен за март 2025 - февраль 2026...")
    raw = yf.download(symbols, start='2025-03-01', end='2026-02-21', interval='1d',
...
print(f"\nКандидатов по чек-пойнтам ({len(checkpoints)} циклов): {counts}")
...
print(f"(эпизод №1, апр-сен 2026: среднее 8.75, медиана 7, мин/макс 1/24, 4 из 16 <5)")
