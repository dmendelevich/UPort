...
# ─── ПОТОЛОК ЗДРАВОГО СМЫСЛА ДЛЯ РАНЖИРОВАНИЯ, НОРМИРОВАННОГО НА ВОЛАТИЛЬНОСТЬ ───
# (Claude/BACKLOG.md, 2026-09-02, живой случай TECH/Bio-Techne) -- обе формулы
# ранжирования (Трендовая: отрыв_от_SMA200/волатильность; Револьверная: глубина_
# просадки/волатильность) делят на signal_daily_volatility_pct. Уже был известный
# риск деления на почти-ноль (LSE застрявшие котировки, BACKLOG.md №94, ranking=178
# у ITRK.L против 24 у второго места) -- тогда защитой был только us_only-фильтр
# (LSE физически исключены). TECH — американская бумага NASDAQ под поглощением
# Merck KGaA ($73/акция, объявлено 25.06.2026): рынок прижал цену к цене сделки,
# волатильность рухнула до 0.2178%/день, а отрыв от SMA200 остался от старого
# скачка (19%) -- ranking=87.3 при нормальном диапазоне 2-25 у настоящих сильных
# кандидатов (DELL≈19, PNC≈24 -- находка Б, HWM/WMT/AVGO≈4-5 у Револьверной).
# Гейт "здравого смысла" по САМОМУ ОТНОШЕНИЮ (не по сырой волатильности -- та у
# облигационных/индексных ETF (VTI/SPY/BND) тоже низкая, но законно, без искажения
# ranking) ловит именно вырожденный случай, независимо от причины (M&A, застрявшая
# котировка, что угодно ещё) -- переиспользуемый механизм, не заплатка под TECH.
TREND_MOMENTUM_RATIO_CEILING = 30.0
REVOLVER_DEPTH_RATIO_CEILING = 20.0
...
# ─── ШПАРГАЛКА «ОПТИМАЛЬНАЯ ЦЕНА» (Claude/BACKLOG.md №122/123, 2026-08-17) ───
# Вторая из двух шпаргалок при решении купить/продать (первая -- EXECUTION_TIMING выше,
# "как можно скорее"). НЕ то же K, что у стоп-лосса/трейлинга -- та задача "тревога",
# эта -- "разумная скидка от ТЕКУЩЕЙ цены, ради которой стоит подождать". Не зависит от
# внутридневного движения (работает и при закрытом рынке) и не требует держащейся позиции
# (для входа avg_price ещё нет) -- отсчитывается от цены на момент решения.
# Откалибровано бэктестом (50 тикеров S&P500, 2 года дневных данных): K=1.25 даёт 66.2%
# шанс коснуться цели за 10 торговых дней (медиана 2 дня) -- заметная, не шумовая скидка,
# не бесконечное ожидание.
OPTIMAL_PRICE_VOLATILITY_MULTIPLIER = 1.25
