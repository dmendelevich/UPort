...
- [19_price_move_protection_design.md](19_price_move_protection_design.md) — «спасай деньги» (продолжение №1/№3/№65/№87): три разных сигнала на резкое движение цены — момент исполнения сделки (A), защита капитала от стратегии (B, стоп-лосс/трейлинг), обвал рынка/сектора через VTI/SPDR (C). Сигнал (B) — в проде; сигнал (A) — ядро реализовано; сигнал (C) — только принципы. Полный ход обсуждения и все находки (включая тупик `getHloc`)
- [20_price_move_protection_continuation.md](20_price_move_protection_continuation.md) — точка входа для ПРОДОЛЖЕНИЯ той же темы в новом диалоге: статус, три next steps по сигналу (A), что не забыть
- [21_paper_broker_emulation_prep.md](21_paper_broker_emulation_prep.md) — точка входа для НОВОЙ темы «эмуляция брокера»: вместо подгонки кода приложения под бумажный портфель — держать код идентичным реальному и эмулировать снизу только брокера (заявка/исполнение/кэш). Полный аудит различий ADVISORY/CONFIRM, набросок целевой архитектуры, открытые вопросы — код не начат
- [22_deal_flow_followups.md](22_deal_flow_followups.md) — тема №21 полностью реализована и протестирована (код V02, см. `BACKLOG.md` №117-130): единая кнопка «🤝 К сделке» для реального и бумажного портфеля. Точка входа для СЛЕДУЮЩЕЙ сессии — хвосты, найденные по ходу и сознательно не исправленные, отложенные темы, и неозвученные «мелкие вопросы» пользователя, которые нужно спросить первым делом
