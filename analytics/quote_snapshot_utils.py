...
def record_quote_snapshot(db_instance, listing_id: int, price: float, buffer_size: int):
    ...
    sql_insert = f"""
        ...
        VALUES ({int(listing_id)}, {float(price)}, (CURRENT_TIMESTAMP AT TIME ZONE 'UTC')::timestamp(0));
    """
    db_instance.execute_query(sql_insert)

    sql_prune = f"""
        DELETE FROM public.quote_snapshots
        WHERE listing_id = {int(listing_id)} AND id NOT IN (
    ...
    """
    db_instance.execute_query(sql_prune)
...
def compute_price_move(db_instance, listing_id: int, current_price: float, window_minutes: int):
    ...
    ref_row = db_instance.execute_row(f"""
        SELECT price,
        ...
        WHERE listing_id = {int(listing_id)}
          AND recorded_at <= (CURRENT_TIMESTAMP AT TIME ZONE 'UTC')::timestamp(0) - INTERVAL '{int(window_minutes)} minutes'
