...
async def process_view_digest(callback: types.CallbackQuery, callback_data: MenuAction, state: FSMContext):
    ...
    Вызывается как вкладка «✅ Дела» с карточки портфеля (bot_handlers/portfolios.py)
    и карточки стратегии (bot_handlers/strategies.py) -- тема «дайджест как вкладка»,
    2026-08-14/15 (переименована в «Дела» 2026-08-20, см. Claude/BACKLOG.md -- отличать
    от утреннего пуша, тот в перспективе станет отдельным нарративным обзором со своей
    кнопкой-переходом сюда), заменила отдельный пикер портфелей из главного меню.
    strategy_id != 0 -- сузить до одной стратегии (filter_digest_data_by_strategy);
    portfolio_id всегда известен -- считаем полный дайджест портфеля один раз, фильтр
    по стратегии -- только презентационный, поверх уже готовых данных.

    Хедер -- ТОТ ЖЕ САМЫЙ стандартный хедер, что и у остальных вкладок этого экрана
    (format_portfolio_header / format_strategy_header+format_strategy_capital_block,
    2026-08-15) -- вкладка «Дела» не должна визуально отличаться от Состав/Паспорт/
