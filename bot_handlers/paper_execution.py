...
def _resolve_trim_shares(trim_qty_raw: float, held_qty: float) -> int:
    """
    PortfolioRebalancer считает trim_qty непрерывно (для реальных П10/П136 --
    просто текст, человек сам округляет на глаз у брокера). «ПБум» же нигде не
    оперирует дробными акциями (см. _execute_virtual_buy) -- округляем здесь,
    ЕДИНСТВЕННОМ месте, где это число превращается в настоящую запись в СУБД, не
    трогая саму PortfolioRebalancer (общая с реальными портфелями).
...
async def _execute_virtual_trim(p_id: int, l_id: int, s_id: int, ticker_id: int, asset_id: int, trim_shares: int, avg_price: float):
    """
    Частичная продажа -- в отличие от _execute_virtual_sell, НЕ удаляет assets,
    а уменьшает quantity/allocated_quantity. avg_price/peak_price_since_entry/
    position_opened_at не трогаются -- себестоимость и история ОСТАВШИХСЯ акций
    не меняются, это тот же самый холдинг, просто меньше. watchlist не трогаем
    (bought_at уже верен, позиция всё ещё держится, sold_out неприменим).
    Возвращает (True, price, proceeds, pnl, pnl_pct, remaining_qty) или (False, error_text).
    """
    price_row = await asyncio.to_thread(
        db_sys.execute_row,
        "SELECT l.last_price FROM public.listings l WHERE l.id = %s;",
    ...
    pnl = (price - avg_price) * trim_shares
    pnl_pct = ((price - avg_price) / avg_price * 100.0) if avg_price > 0 else 0.0
    now = _system_now()

    sql = """
        WITH updated_asset AS (
            UPDATE public.assets SET quantity = quantity - %s, last_updated = %s
            WHERE id = %s
        ),
        updated_strategy_asset AS (
            UPDATE public.strategy_assets SET allocated_quantity = allocated_quantity - %s, last_updated_at = %s
            WHERE asset_id = %s AND strategy_id = %s
        ),
        new_pipeline AS (
            INSERT INTO public.order_pipelines
                (portfolio_id, listing_id, strategy_id, ticker_id, current_step, pipeline_status, target_quantity, initial_entry_price, is_manual_override)
            VALUES (%s, %s, %s, %s, 1, 'COMPLETED', %s, 0, false)
        ),
        updated_cash AS (
            UPDATE public.accounts SET cash_available = cash_available + %s, last_updated = %s
            WHERE portfolio_id = %s AND currency_id = 'USD'
    ...
    """
    params = (
        trim_shares, now, asset_id,
        trim_shares, now, asset_id, s_id,
        p_id, l_id, s_id, ticker_id, -trim_shares,
        proceeds, now, p_id,
    )
    await asyncio.to_thread(db_sys.execute_query, sql, params)
    await asyncio.to_thread(db_sys.execute_query, _refresh_assets_value_sql(), (p_id, p_id))

    remaining_row = await asyncio.to_thread(db_sys.execute_row, "SELECT quantity FROM public.assets WHERE id = %s;", (asset_id,))
    remaining_qty = float((remaining_row or {}).get("quantity") or 0.0)
...
async def send_paper_trim_recommendations(db_instance, bot):
    """
    Третий тип подтверждения «ПБум» (BACKLOG.md №108/№111) -- частичная подрезка
    перевешенной позиции (recommendation='TRIM_DOWN' из PortfolioRebalancer,
    сегодня только Консервативная, см. её докстринг). В отличие от BUY/SELL --
    trim_qty из PortfolioRebalancer непрерывный (общий источник с текстом
    реальных П10/П136) -- округляется до целых акций и защищается "оставить
    хотя бы 1" именно здесь, единственном потребителе, который реально исполняет
    число, а не просто печатает его (см. _resolve_trim_shares).
    """
    ...
    for p in portfolios:
        ...
        try:
            alerts = await asyncio.to_thread(PortfolioRebalancer(db_instance).evaluate_portfolio, p_id)
        except Exception as e:
            logging.error(f"❌ [PaperExec]: Не удалось посчитать рекомендации подрезки для '{p_name}' (ID: {p_id}): {e}")
            error_count += 1
            continue

        for alert in alerts:
            if alert.get("recommendation") != "TRIM_DOWN":
                continue

            asset_id = int(alert["asset_id"])
            l_id = int(alert["listing_id"])
            s_id = int(alert["strategy_id"])
            symbol = alert["symbol"]

            asset_row = await asyncio.to_thread(db_instance.execute_row, "SELECT quantity FROM public.assets WHERE id = %s;", (asset_id,))
            held_qty = float((asset_row or {}).get("quantity") or 0.0)
            trim_shares = _resolve_trim_shares(float(alert["quantity"]), held_qty)
            if trim_shares <= 0:
                # Позиция слишком маленькая, чтобы частично подрезать (напр. 1 шт) --
                # не предлагаем действие, но и не молчим полностью: текстовая строка
                # с той же причиной уже есть в обычном разделе "Лимиты" дайджеста.
                continue

            text = (
                f"✂️ *{p_name}* — {alert['strategy_name']}\n"
                f"Позиция: *{symbol}* ({trim_shares:g} из {held_qty:g} шт)\n\n"
                f"{alert['reason']}\n\n"
                f"Подрезать виртуально?"
            )
            keyboard = generate_confirm_keyboard(
                yes_text="✅ Да",
                yes_callback_packed=MenuAction(action="paper_trim_down_yes", portfolio_id=p_id, listing_id=l_id, strategy_id=s_id).pack(),
                no_text="❌ Нет",
                no_callback_packed=MenuAction(action="paper_trim_down_no", portfolio_id=p_id, listing_id=l_id, strategy_id=s_id).pack(),
            )
            try:
                await bot.send_message(chat_id=chat_id, text=text, parse_mode="Markdown", reply_markup=keyboard)
                logging.info(f"✅ [PaperExec]: Рекомендация подрезки {symbol} для '{p_name}' отправлена на подтверждение.")
            except Exception as e:
                logging.error(f"❌ [PaperExec]: Не удалось отправить рекомендацию подрезки {symbol} для '{p_name}': {e}")
                error_count += 1

    return {"processed": len(portfolios), "errors": error_count}


@router.callback_query(MenuAction.filter(F.action == "paper_trim_down_no"))
async def process_paper_trim_down_no(callback: types.CallbackQuery, callback_data: MenuAction):
    """Отказ -- ничего не пишем в БД, отказ не запоминается (как и у BUY/SELL)."""
    await callback.answer()
    try:
        await callback.message.edit_text(
            "❌ Отклонено.",
...
@router.callback_query(MenuAction.filter(F.action == "paper_trim_down_yes"))
async def process_paper_trim_down_yes(callback: types.CallbackQuery, callback_data: MenuAction):
    """
    Подтверждение подрезки -- перепроверяет условия заново (позиция могла уже не
    быть перевешенной, если цена откатилась с утра) и пересчитывает trim_shares
    от ЖИВОГО количества акций, не от утреннего сообщения.
    """
    p_id = callback_data.portfolio_id
    l_id = callback_data.listing_id
    s_id = callback_data.strategy_id
    ...
    alerts = await asyncio.to_thread(PortfolioRebalancer(db_sys).evaluate_portfolio, p_id)
    match = next(
        (a for a in alerts
         if a.get("recommendation") == "TRIM_DOWN" and int(a.get("listing_id", -1)) == l_id and int(a.get("strategy_id", -1)) == s_id),
        None
    )
    if not match:
        try:
            await callback.message.edit_text(
                "⚠️ Условия изменились, подрезка больше не актуальна.",
                reply_markup=_back_keyboard(p_id, l_id)
            )
        except TelegramBadRequest:
            pass
    ...
    asset_row = await asyncio.to_thread(db_sys.execute_row, "SELECT quantity, avg_price FROM public.assets WHERE id = %s;", (asset_id,))
    held_qty = float((asset_row or {}).get("quantity") or 0.0)
    avg_price = float((asset_row or {}).get("avg_price") or 0.0)
    trim_shares = _resolve_trim_shares(float(match["quantity"]), held_qty)
    if trim_shares <= 0:
        try:
            await callback.message.edit_text(
                "⚠️ Условия изменились, подрезка больше не актуальна.",
    ...
    result = await _execute_virtual_trim(p_id, l_id, s_id, ticker_id, asset_id, trim_shares, avg_price)
    if not result[0]:
        try:
            await callback.message.edit_text(result[1], reply_markup=_back_keyboard(p_id, l_id))
        except TelegramBadRequest:
    ...
    _, price, proceeds, pnl, pnl_pct, remaining_qty = result
    logging.info(
        f"✅ [PaperExec]: Виртуально подрезано {trim_shares} шт {symbol} по ${price:,.2f} из '{strategy_name}' "
        f"(портфель {p_id}), P&L ${pnl:,.2f}, осталось {remaining_qty:g} шт."
    )
    pnl_sign = "+" if pnl >= 0 else ""
    try:
        await callback.message.edit_text(
            f"✅ Подрезано: {trim_shares:g} шт *{symbol}* по ${price:,.2f} (${proceeds:,.2f})\n"
            f"P&L на проданной части: {pnl_sign}${pnl:,.2f} ({pnl_sign}{pnl_pct:.1f}%)\n"
            f"Осталось: {remaining_qty:g} шт.",
            parse_mode="Markdown",
            reply_markup=_back_keyboard(p_id, l_id)
        )
    except TelegramBadRequest:
